...
stock_list = list(final_data['symbols'])

data = yf.download(tickers=stock_list, progress=False) #Take around 2-3 min to run, the data will be in dataframe form which can be acess
# by using this form 
# data[('a', label)] a can be 'Close', 'Open', 'Adj Close', 'Volume', 'High' and 'Low'
# and Label is a stock ticker for example 'True.BK'
# If you want to plot a graph here is an example; plt.plot(data[('Close', 'TRUE.BB')])